D:\mailCloud\prjother\tmp\1\c23_Converting a Decimal to binaryhex.md  


__|__
--|--
[BLANK_AUDIO]|[BLANK_AUDIO]
All right ladies and gentleman, now we're gonna go ahead and convert a decimal to binary to hex.|Хорошо, дамы и господа, теперь мы собираемся преобразовать десятичное число в двоичное в шестнадцатеричное.
We'll do just one.|Мы сделаем только один.
It's the same concept pretty much.|Это почти та же концепция.
So, I mean, since you're not doing it on the test, we'll just do it real quick.|Итак, я имею в виду, раз вы не делаете этого на тесте, мы просто сделаем это очень быстро.
But I'm not gonna make up the number, so you don't think I'm the one that's actually, you know,|Но я не собираюсь придумывать номер, так что ты не думаешь, что я тот, кто на самом деле, ты знаешь,
already have a number made up, and I'm just doing it.|у меня уже есть номер, и я просто делаю это.
So I'm gonna ask, gimme a number, anybody,|Итак, я собираюсь спросить, дайте мне номер, кто угодно,
anybody?|кто-нибудь?
Anybody yell a number.|Кто-нибудь кричит номер.
You right there, give me, gimme a number.|Вы прямо здесь, дайте мне, дайте мне номер.
 238.| 238.
 238?| 238?
Okay.|Ладно.
No problem.|Нет проблем.
No problem.|Нет проблем.
The shy girl in the back.|Застенчивая девушка сзади.
238.|238.
With the Mickey Mouse, Minnie Mouse voice.|С Микки Маусом, голосом Минни Маус.
All right, so what do you do?|Хорошо, так что ты делаешь?
All right, there's many ways of doing it.|Хорошо, есть много способов сделать это.
You have a decimal.|У вас есть десятичная дробь.
I'll get rid of these numbers here,|Я избавлюсь от этих чисел здесь,
these binary bit values right here from the previous lesson.|эти двоичные битовые значения прямо здесь из предыдущего урока.
All right?|Отлично?
We have a number 238, I'm gonna center it.|У нас есть номер 238, я собираюсь его центрировать.
So, use your table.|Итак, используйте свою таблицу.
Where is it?|Где это находится?
Here it is, 238.|Вот оно, 238.
How close can we get?|Как близко мы можем подойти?
240.|240.
Well no, that's too much.|Нет, это уж слишком.
224, right?|224, верно?
224, so we got, we got one, one, one.|224, так что у нас есть, у нас есть один, один, один.
So, we know that we have 224, so how many more do we need to get to 238?|Итак, мы знаем, что у нас 224, так сколько еще нам нужно, чтобы получить 238?
That's, 224.|Это 224.
We would need, how's it going?|Нам понадобится, как дела?
Okay so, let's subtract.|Ладно, давайте вычтем.
Let's see, we put the number in here, 224.|Посмотрим, здесь мы указываем число 224.
Wow, that'll be eight, that will be 14,|Вау, это будет восемь, это будет 14,
sorry, I had to visually see it.|извините, мне пришлось это визуально увидеть.
We need 14 more.|Нам нужно еще 14.
So definitely 16, way too much, eight,|Так что определенно 16, слишком много, восемь,
okay.|Ладно.
And four, that's 12.|И четыре, это 12.
And two, that's 14.|И два, это 14.
And then zero.|А потом ноль.
Oh.|Ой.
So that will come out to 238.|Так что получится 238.
So you have 224 and, and and it's a,|Итак, у вас есть 224 и, и это,
at the eight and the two, that's ten,|в восемь и два, это десять,
that's 234, plus four more.|это 234 плюс еще четыре.
That's 238, so that is your binary bit.|Это 238, так что это ваш двоичный бит.
Remember, everything goes to binary, so we took the 238,|Помните, все идет в двоичном формате, поэтому мы взяли 238,
...
that the programmers and the web designers are the ones that are actually looking forward to this hex conversion thing.|что программисты и веб-дизайнеры на самом деле с нетерпением ждут этого шестнадцатеричного преобразования.
